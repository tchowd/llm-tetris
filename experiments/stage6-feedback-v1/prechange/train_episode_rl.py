...
discounted reward-to-go at each turn, and normalizes advantages only across
the same-start group at that turn.
"""
...
def main() -> None:
    ...
    parser.add_argument("--experiment", choices=("E5", "E6", "E7"), required=True)
    ...
    for update in range(start_update + 1, args.updates + 1):
        ...
        advantages = trajectory_advantages(rewards, args.gamma)
        ...
        for offset in range(0, len(steps), chunk_size):
            ...
            action_losses = []
            action_kls = []
            for row, current_tokens in zip(chunk, token_rows):
                reference_tokens = torch.tensor(row["reference_token_logprobs"], dtype=current_tokens.dtype, device=current_tokens.device)
                advantage = torch.full_like(current_tokens, row["advantage"])
                action_losses.append(grouped_policy_loss(current_tokens, reference_tokens, advantage, args.kl_beta))
                delta = reference_tokens - current_tokens
                action_kls.append((torch.exp(delta) - delta - 1.0).mean())
            loss = torch.stack(action_losses).sum() / len(steps)
            loss.backward()
            total_loss += float(loss.detach().cpu())
            total_kl += float(torch.stack(action_kls).detach().sum().cpu()) / len(steps)
