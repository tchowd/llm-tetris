...
async function renderAws(content) {
  ...
  const paths = ["resources", "jobs", "metrics", "logs?limit=50", "costs", "credits", "quotas", "alarms", "security"];
  ...
  const [resourcesP, jobsP, metricsP, logsP, costsP, creditsP, quotasP, alarmsP, securityP] = results;
  ...
  content.innerHTML = `${pageHeader("Direct AWS read APIs", "AWS operations", "Compute, jobs, utilization, cost, credits, quota, and IAM posture. Every panel fails independently.", `<span class="live-state"><i></i>30s resource cache</span>`)}
    ...
    <div class="content-grid equal">
      ...
      <section class="panel"><div class="panel-heading"><div><span class="section-kicker">Security & capacity</span><h2>Guardrails</h2></div></div>${panelError([...(securityP.errors || []), ...(creditsP.errors || []), ...(quotasP.errors || []), ...(alarmsP.errors || [])])}<div class="guardrail-list"><div><span>Credits</span><strong>${creditsP.data ? "Available" : "Unknown"}</strong><small>Billing GetCredits</small></div><div><span>GPU quota</span><strong>${quotasP.data?.[0]?.value ?? "Unknown"}</strong><small>${quotasP.data?.[0]?.pending_requests?.length ? "request pending" : "no pending request"}</small></div><div><span>Active alarms</span><strong>${activeAlarms.length}</strong><small>${alarms.length} project alarms</small></div><div><span>Principal</span><strong>${esc(security.principal || "Unknown")}</strong><small>${security.policies?.length ?? 0} policies visible</small></div></div>${activeAlarms.map((alarm) => `<div class="security-warning">${icon("alert")}<div><strong>${esc(alarm.name)}</strong><p>${esc(alarm.reason || alarm.description || "CloudWatch alarm is active.")}</p></div></div>`).join("")}${(security.warnings || []).map((warning) => `<div class="security-warning">${icon("alert")}<div><strong>${esc(warning.title)}</strong><p>${esc(warning.next_action)}</p></div></div>`).join("")}</section>
    </div>`;
}
