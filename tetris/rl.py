...
def trajectory_advantages(reward_groups: Sequence[Sequence[float]], gamma: float = 0.99,
                          *, method: str = "active_group", reward_scale: float = 10.0) -> list[list[float]]:
    """Historical active-group normalization or fixed-zero-baseline returns.

    ``fixed_zero`` uses an action-independent zero baseline and a fixed positive
    scale, not a sign override for failures. See the prospective feedback spec.
    """

    if method not in {"active_group", "fixed_zero"}:
    ...
    if any(not math.isfinite(value) for row in reward_groups for value in row):
    ...
    if any(not math.isfinite(value) for row in returns for value in row):
        raise ValueError("reward-to-go overflow")
    if method == "fixed_zero":
        values = [[value / reward_scale for value in row] for row in returns]
        if any(not math.isfinite(value) for row in values for value in row):
            raise ValueError("scaled reward-to-go overflow")
        return values
...
def advantage_diagnostics(trajectories: Sequence[dict], threshold: float = 1e-10) -> dict:
    """Counts with explicit denominators, suitable for aggregation across updates."""
    if not math.isfinite(threshold) or threshold <= 0:
        raise ValueError("zero threshold must be positive and finite")
    lengths = [len(row["steps"]) for row in trajectories]
    buckets = {name: [] for name in ("all", "terminal_illegal", "terminal_topout", "single_active", "single_active_illegal")}
    for trajectory in trajectories:
        for turn, step in enumerate(trajectory["steps"]):
            value = step["advantage"]
            if not math.isfinite(value):
                raise ValueError("non-finite advantage")
            single = sum(length > turn for length in lengths) == 1
            illegal = step.get("terminal_reason") == "illegal_action"
            buckets["all"].append(value)
            if illegal:
                buckets["terminal_illegal"].append(value)
            if step.get("terminal_reason") == "topped_out":
                buckets["terminal_topout"].append(value)
            if single:
                buckets["single_active"].append(value)
            if single and illegal:
                buckets["single_active_illegal"].append(value)
    return {"threshold": threshold, **{name: {
        "count": len(values), "exact_zero": sum(v == 0 for v in values),
        "effective_zero": sum(abs(v) < threshold for v in values),
        "negative": sum(v < -threshold for v in values),
