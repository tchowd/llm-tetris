#!/usr/bin/env bash
# E2 v2: the sole registered initialization repair; preserve v1's failed eligibility.
...
run_dir=runs/rl-e2v2-seed0/rl
weak_dir=runs/rl-e2v2-weak-sft/rl
...
if [[ ${2:-} != --workflow ]]; then
  ...
  finish() {
    ...
    for run in rl-e2v2-weak-sft rl-e2v2-seed0; do
  ...
  trap finish EXIT
  phase starting
  timeout --signal=INT --kill-after=120s 340m bash "$0" "$prior" --workflow > "$run_dir/block.log" 2>&1
  exit 0
fi

phase weakened_sft_training
timeout --signal=INT --kill-after=120s 90m "$py" scripts/train_sft.py \
  --data-dirs data/batch1 data/batch2 --out-dir "$weak_dir" \
  --base-model-revision 70d244cc86ccca08cf5af4e1e306ecf908b1ad5e \
  --backend hf --device cuda --max-train-rows 65536 --max-steps 4096 \
  --batch-size 4 --grad-accum 4 --lr 0.0001 --seed 0 \
  --max-eval-rows 256 --gen-eval-rows 64 --eval-steps 512 --save-steps 512 \
  --logging-steps 10 > "$weak_dir/train.log" 2>&1
"$py" -c 'import json; from pathlib import Path; m=json.loads(Path("runs/rl-e2v2-weak-sft/rl/train_manifest.json").read_text()); assert m["status"]=="passed" and m["total_steps"]==4096 and m["num_train_rows"]==65536 and m["seed_applied_before_model_init"]; assert m["base_model_revision"]=="70d244cc86ccca08cf5af4e1e306ecf908b1ad5e"'
...
timeout --kill-after=15s 5m "$py" scripts/sync_run_artifacts.py upload --run-id rl-e2v2-weak-sft --include-adapter --receipt "$weak_dir/sync-receipt.json"
timeout --kill-after=15s 2m "$py" scripts/sync_run_artifacts.py upload --run-id rl-e2v2-seed0

phase dense_rl_training
# Prior training budget allowance includes up to 180 minutes already spent in this block.
rl_prior=$("$py" -c 'import sys; print(float(sys.argv[1])+180/60*1.05)' "$prior")
...
"$py" -c 'import json; from pathlib import Path; m=json.loads(Path("runs/rl-e2v2-seed0/rl/manifest.json").read_text()); assert m["status"]=="completed" and m["completed_updates"]==512 and m["reference_frozen"]'
